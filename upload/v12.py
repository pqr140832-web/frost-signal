...
整个项目最关键的突破就是认识到这个赛题的本质不是插值而是外推。
之前所有版本不管是RandomForest还是XGBoost做多模型融合，OOF交叉验证的R²都很高（v6达到0.92），
但提交上去分数反而不如预期。后来想明白了，交叉验证是把已有时间点拆开再拼回去，本质上是插值，
而测试集要求的是对更远时间点的预测，这完全是两回事。所以v7做了一个思维转变，不再把所有数据扔进一个黑盒模型去学，
而是回归物理本质：颜料老化的色差变化遵循扩散定律，dE∝√t。具体做法是对每个样品在每种老化条件下单独提取时间序列，
用√t拟合加上最后两个点的线性趋势外推做加权组合，这个纯物理方法在前向验证上直接做到了R²=0.96，比之前所有机器学习方案高不少。
在这个核心突破之上，v12做了进一步精细化。它发现不同颜料的老化行为模式完全不一样，不能都用固定的√t，
于是用坐标下降法搜索出每组的最优幂次n，钴蓝增长极慢n只有0.3，曙红接近线性n=0.85，矿物颜料后期加速反而n=1.2超线性。
同时v12也对两个子模型的融合权重做了分组搜索，噪声大的翡翠绿更依赖物理模型（权重0.75），
趋势稳定的曙红几乎完全靠线性外推（权重0.05）。再加上异常点过滤机制去掉噪声点，
最终v12把R²从v7的0.96提升到了0.9693，MAE从0.40降到了0.3244。
"""
