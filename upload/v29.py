...
核心改进：
1. 添加sqrt模型 (y = A*sqrt(t) + B)
2. 曙红组：加入"回归均值"效应，远距离外推时个体缩放因子逐渐趋向1
3. 皮纸组：使用更保守的饱和外推策略
4. 组级通道分解辅助：用通道级趋势验证dE预测方向
5. LOLO引导的权重调优：对每组使用多个策略候选，选最优组合
"""
        ...
        if p_group is not None and len(tc) >= 1:
            ...
            if group_median_key in self.robust_model.group_medians:
                ...
                if group_val_at_t > 0.01:
                    scale_factor_raw = dEc[-1] / group_val_at_t
                    p_scaled_group = p_group * scale_factor_raw

        # 策略6: 回归均值版缩放 (远距离外推时缩放因子趋向1)
        p_rtm_group = None
        if p_group is not None and scale_factor_raw is not None and len(tc) >= 1:
            # 计算外推距离占训练时间跨度的比例
            t_span = tc[-1] - tc[0] if tc[-1] > tc[0] else 1.0
            ext_ratio = max(0, (t_pred - tc[-1])) / t_span
            # 缩放因子从scale_factor_raw逐渐趋向1
            # 使用指数衰减: scale = 1 + (raw_scale - 1) * exp(-lambda * ext_ratio)
            lam = 0.5  # 衰减速率
            rtm_scale = 1.0 + (scale_factor_raw - 1.0) * np.exp(-lam * ext_ratio)
            p_rtm_group = p_group * rtm_scale

        # 策略7: 线性+饱和混合 (用于远距离外推)
        p_hybrid = None
        if p_linear is not None and p_group is not None:
            # 对远距离外推，线性外推可能过高，组模型可能偏低
            # 取加权平均
            ext_ratio = max(0, (t_pred - tc[-1])) / (tc[-1] - tc[0]) if tc[-1] > tc[0] else 0
            # 近距离偏线性，远距离偏组模型
            w_lin = max(0, 1.0 - ext_ratio)
            w_grp = 1.0 - w_lin
            p_hybrid = w_lin * p_linear + w_grp * p_group

        return self._select_strategy(group, tc, dEc, t_pred,
                                     p_individual, p_group, p_channel,
                                     p_linear, p_scaled_group, p_rtm_group, p_hybrid)

    def _select_strategy(self, group, t_train, dE_train, t_pred,
                         p_ind, p_grp, p_ch, p_lin, p_scaled, p_rtm, p_hybrid):
        if group == "cobalt_blue":
            # 钴蓝：噪声大，通道分解+组模型为主
            if p_ch is not None and p_grp is not None:
                # v28核心: 0.5*ch + 0.3*grp + 0.2*scaled
                scaled = p_scaled if p_scaled is not None else p_grp
                return 0.5 * p_ch + 0.3 * p_grp + 0.2 * scaled
        ...
        if group == "shu_red":
            ...
            if p_rtm is not None:
                candidates.append(p_rtm)
                weights.append(2.0)  # 回归均值版缩放，最高权重
            if p_scaled is not None:
                candidates.append(p_scaled)
                weights.append(1.5)  # 原始缩放
            if p_ind is not None:
                candidates.append(p_ind)
                weights.append(0.8)  # 个体模型
            if p_grp is not None:
                candidates.append(p_grp)
                weights.append(0.5)  # 组模型
            if p_ch is not None:
                candidates.append(p_ch)
                weights.append(0.3)

            if not candidates:
                return p_lin if p_lin is not None else 1.0
            wsum = sum(weights)
            return sum(c * w for c, w in zip(candidates, weights)) / wsum
        ...
        if group == "paper":
            ...
            if p_lin is not None:
                candidates.append(p_lin)
                weights.append(0.3)

            if not candidates:
                return p_lin if p_lin is not None else 0.0
            wsum = sum(weights)
            return sum(c * w for c, w in zip(candidates, weights)) / wsum
...
def true_lolo_eval(df_train):
    ...
    for sample in df_train["sample"].unique():
        for cond in df_train[df_train["sample"] == sample]["aging_condition"].unique():
            ...
            predictor = V29Predictor(train_df)
...
def main():
    ...
    print("\n[LOLO详情] 钴蓝组:")
    for d in metrics["details"]:
        if d["group"] == "cobalt_blue":
            print(f"  {d['sample']:20s} t={d['t']:5.0f}  true={d['y_true']:.4f}  pred={d['y_pred']:.4f}  err={abs(d['y_true']-d['y_pred']):.4f}")

    print("\n[预测] 测试集...")
    predictor = V29Predictor(df_train)
    test_preds = []
    ...
    print(f"\nv29 LOLO: R²={metrics['R2']:.4f}, MAE={metrics['MAE']:.4f}")

    # 保存到download
    download_dir = Path("/home/z/my-project/download")
    pd.DataFrame({TARGET: test_preds}).to_csv(download_dir / "predict_out_v29.csv", index=False)
    print(f"[保存] {download_dir / 'predict_out_v29.csv'}")
