"""
颜料老化色差预测 - v34b
基于v33框架 + 新模型 + 物理约束 + 偏差修正

核心改进（保守但稳健）：
1. 新增Weibull增长模型: dE = A*(1-exp(-(t/λ)^k)) - 纸张等饱和老化
2. 新增Gompertz模型: dE = A*exp(-B*exp(-k*t)) - S型老化
3. 单调性约束: dE不随时间递减
4. 偏差修正: 基于LOLO的系统性偏差修正（保守应用）
5. 曙红组特殊处理: 曙红7异常检测+修正
6. 保持v33的权重搜索方式（已验证有效）
"""
...
def best_model_with_floor(t, y, floor_val=0):
    """获取最佳模型，但预测值不低于floor_val（单调性约束）"""
    m = best_model(t, y)
...
def search_best_weights(lolo_df):
    """基于v33的搜索方式: 大量随机搜索"""
    np.random.seed(42)
...
def main():
    print("=" * 60)
    print("  颜料老化色差预测 v34b")
    print("  Weibull/Gompertz + 单调性约束 + 偏差修正")
    print("=" * 60)
    ...
    # 搜索最优权重
    print("\n[搜索] LOLO最优权重...")
    best_weights = search_best_weights(lolo_df)

    # 曙红组手动修正
    print("\n[修正] 曙红组权重...")
    print(f"  搜索得到: {best_weights['shu_red']}")
    # 曙红在测试集中外推到t=24/30（训练到18），数据少，个体模型不可靠
    # 混合scaled+ch_scaled+grp+ind更安全
    shu_fixed = {sn: 0.0 for sn in STRATEGY_NAMES}
    shu_fixed["scaled"] = 0.35
    shu_fixed["ch_scaled"] = 0.20
    shu_fixed["grp"] = 0.20
    shu_fixed["lin"] = 0.10
    shu_fixed["ind"] = 0.15
    best_weights["shu_red"] = shu_fixed
    print(f"  修正后: {best_weights['shu_red']}")

    # 最终LOLO评估
    print("\n[LOLO评估]:")
    final_mae, final_ge = eval_weights(lolo_df, best_weights)
    print(f"  总体 MAE = {final_mae:.4f}")
    for g, errs in final_ge.items():
        if len(errs) >= 2:
    ...
    for _, row in lolo_df.iterrows():
        g = row["group"]
        w = best_weights.get(g, {sn: 1/6 for sn in STRATEGY_NAMES})
        ws, wp = 0, 0
        for sn in STRATEGY_NAMES:
            v = row[sn]
            if v is not None and not (isinstance(v, float) and np.isnan(v)):
    ...
    for g in ["dye", "paper", "shu_red", "jade_green", "cobalt_blue"]:
        ...
        for _, row in gdf.iterrows():
            ...
            if ws > 0:
                preds.append(wp / ws)
                trues.append(row["y_true"])
        preds, trues = np.array(preds), np.array(trues)
        if np.mean(preds) > 0.1:
            ratio = np.mean(trues) / np.mean(preds)
            bias_corrections[g] = ratio
            print(f"  {g:15s}: ratio={ratio:.4f} (mean_pred={np.mean(preds):.3f}, mean_true={np.mean(trues):.3f})")
    ...
    for _, row in df_test.iterrows():
        ...
        # 应用偏差修正（保守：只修正方向合理的）
        if g in bias_corrections:
            bc = bias_corrections[g]
            # 只在修正不超过10%时应用
            if 0.9 <= bc <= 1.1:
                pred = pred * bc
        ...
        if "曙红7" in row["sample"]:
            # 曙红7训练数据显示dE在t=12和t=18都是正的
            # 如果预测值低于1.0，用曙红组中位数预测
            if pred < 1.0:
                sub_shu = df_train[(df_train["sample"].str.contains("曙红")) & (df_train["aging_condition"] == "UV")]
                shu_med = sub_shu[sub_shu["aging_time_day"] > 0][TARGET].median()
                # 用曙红组内其他样本的缩放预测
                ...
                if len(t_arr) >= 2 and len(dE_arr) >= 2:
                    ...
                    for s in sub_shu["sample"].unique():
                        ...
                        if len(tt) >= 2 and len(dd) >= 2:
                            ratio = dd[-1] / max(dd[-2], 0.01)
                            all_shu_dE.append((s, ratio, dd[-1]))
                    if all_shu_dE:
                        ratios = [r[1] for r in all_shu_dE]
                        med_ratio = np.median(ratios)
                        pred = last_dE * med_ratio
                        # 如果外推距离超过一个间隔，用幂律
                        t_last = t_arr[-1]
    ...
    pd.DataFrame({TARGET: test_preds}).to_csv(Path("/home/z/my-project/download/predict_out_v34b.csv"), index=False)
    ...
    # 版本对比
    print(f"\n[版本对比]")
    print(f"  v34b: R2={r2:.4f}, MAE={final_mae:.4f}")
    ...
    if v14_path.exists():
        ...
        for w14, label in [(0.3, "0.3v14_0.7v34b"), (0.5, "0.5v14_0.5v34b"), (0.7, "0.7v14_0.3v34b")]:
            ens = w14 * v14 + (1 - w14) * test_preds
            pd.DataFrame({TARGET: ens}).to_csv(dl / f"predict_out_v14_v34b_{label}.csv", index=False)

    for other_v in ["v28", "v32", "v33"]:
        other_path = dl / f"predict_out_{other_v}.csv"
        if other_path.exists():
            other_preds = pd.read_csv(other_path)[TARGET].values
            for w1, label in [(0.5, f"0.5{other_v}_0.5v34b"), (0.7, f"0.7{other_v}_0.3v34b")]:
                ens = w1 * other_preds + (1 - w1) * test_preds
                pd.DataFrame({TARGET: ens}).to_csv(dl / f"predict_out_{other_v}_v34b_{label}.csv", index=False)

    print("\n[ensemble] 已生成")
    print(f"\n[最优权重]")
    for g, w in best_weights.items():
        print(f"  {g:15s}: {dict((k, round(v, 3)) for k, v in w.items())}")
