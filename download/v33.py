...
核心修复：
1. 曙红组：ch/ch_scaled权重过高导致预测值过低
   - 曙红通道外推严重低估，因为通道线性外推后变化很小
   - 改用dE直接外探 + 通道外推的加权
2. 对LOLO权重搜索添加约束：各组MAE不能比v28差太多
3. 皮纸组：保持线性外推主导(已验证有效)
4. 翡翠绿组：保持通道主导(已验证有效)
"""
        ...
        for group in ["dye", "paper", "shu_red", "jade_green", "cobalt_blue", "other"]:
            ...
            for cond in ["UV", "humid-_heat"]:
                ...
                for ch_name in ["dL", "da", "db"]:
                    ...
                    ch_models[ch_name] = {"linear": lin, "abs_model": abs_model, "sign": np.sign(np.median(ch_vals))}
...
def search_best_weights(lolo_df):
    ...
    for group in groups:
        ...
        for trial in range(10000):
            ...
            mask = np.random.random(len(STRATEGY_NAMES)) > 0.2
            raw_w *= mask
...
def main():
    ...
    # 但对曙红组，手动修正权重：增加scaled(individual-scaled group model)
    # 因为曙红ch_scaled在LOLO时只有一个数据点，过拟合了
    print("\n[修正] 曙红组权重...")
    print(f"  搜索得到: {best_weights['shu_red']}")

    # 曙红修正：对曙红，用组级scaled + dE线性外推 + 个体模型的组合
    # 测试集有全部数据(t=12,18)，通道外推应该比LOLO时更准确
    # 但LOLO权重ch_scaled=0.52太高(LOLO时只有1个点，通道外推退化为插值)
    # 全数据时通道外推有效，但仍需保守
    # 修正：增加scaled(组级dE缩放)权重，减少ch_scaled
    shu_fixed = {sn: 0.0 for sn in STRATEGY_NAMES}
    shu_fixed["scaled"] = 0.40
    shu_fixed["ch_scaled"] = 0.25
    shu_fixed["grp"] = 0.20
    shu_fixed["lin"] = 0.10
    shu_fixed["ind"] = 0.05
    ...
    pd.DataFrame({TARGET: test_preds}).to_csv(Path("/home/z/my-project/download/predict_out_v33.csv"), index=False)

    print(f"\n[预测明细]")
    for i, (_, row) in enumerate(df_test.iterrows()):
        print(f"  {row['sample']:20s} ({row['aging_condition']:12s}, t={row['aging_time_day']:3.0f}d) -> {test_preds[i]:.4f}")

    print(f"\nv33: R2={r2:.4f}, MAE={final_mae:.4f}")
    print(f"v28: R2=0.918, MAE=0.626")
    print(f"v32: R2=0.960, MAE=0.375")

    # 生成ensemble
    v14_path = Path("/home/z/my-project/download/predict_out_v14.csv")
    if v14_path.exists():
        v14 = pd.read_csv(v14_path)[TARGET].values
        dl = Path("/home/z/my-project/download")
        for w14, label in [(0.3, "0.3v14_0.7v33"), (0.5, "0.5v14_0.5v33"), (0.7, "0.7v14_0.3v33")]:
            ens = w14 * v14 + (1 - w14) * test_preds
            pd.DataFrame({TARGET: ens}).to_csv(dl / f"predict_out_v14_v33_{label}.csv", index=False)
        # v28+v33
        v28_path = Path("/home/z/my-project/download/predict_out_v28.csv")
        if v28_path.exists():
            v28 = pd.read_csv(v28_path)[TARGET].values
            for w28, label in [(0.5, "0.5v28_0.5v33"), (0.7, "0.7v28_0.3v33")]:
                ens = w28 * v28 + (1 - w28) * test_preds
                pd.DataFrame({TARGET: ens}).to_csv(dl / f"predict_out_v28_v33_{label}.csv", index=False)
        print("\n[ensemble] 已生成")
