"""
颜料老化色差预测 - v32
LOLO引导权重优化 + v28为基础 + 新增策略6(通道缩放)

核心改进：
1. 在v28基础上新增策略6: 个体缩放的通道预测
2. 新增exp_decay饱和模型
3. 通道级外推建模(线性外推通道值)
4. 对每组搜索LOLO最优权重
5. 优化other组处理(几乎不外推)
...
def main():
    ...
    print("\n[v28基线]:")
    v28_w = {
        "cobalt_blue": {"ind": 0.0, "grp": 0.3, "ch": 0.5, "lin": 0.0, "scaled": 0.2, "ch_scaled": 0.0},
        "jade_green": {"ind": 0.3, "grp": 0.2, "ch": 0.3, "lin": 0.0, "scaled": 0.2, "ch_scaled": 0.0},
        "shu_red": {"ind": 0.2, "grp": 0.3, "ch": 0.0, "lin": 0.0, "scaled": 0.5, "ch_scaled": 0.0},
        "dye": {"ind": 0.8, "grp": 0.2, "ch": 0.0, "lin": 0.0, "scaled": 0.0, "ch_scaled": 0.0},
        "paper": {"ind": 0.3, "grp": 0.2, "ch": 0.0, "lin": 0.3, "scaled": 0.2, "ch_scaled": 0.0},
        "other": {sn: 1/6 for sn in STRATEGY_NAMES},
    }
    v28_mae, v28_ge = eval_weights(lolo_df, v28_w)
    print(f"  总体 MAE = {v28_mae:.4f}")
    for g, errs in v28_ge.items():
        if len(errs) >= 2:
    ...
    # 评估最优权重
    print("\n[最优权重] 评估:")
    opt_mae, opt_ge = eval_weights(lolo_df, best_weights)
    print(f"  总体 MAE = {opt_mae:.4f}")
    for g, errs in opt_ge.items():
        if len(errs) >= 2:
            print(f"    {g:15s}: MAE={np.mean(errs):.4f}")
    ...
    pd.DataFrame({TARGET: test_preds}).to_csv(Path("/home/z/my-project/download/predict_out_v32.csv"), index=False)

    print(f"\n[预测明细]")
    for i, (_, row) in enumerate(df_test.iterrows()):
        print(f"  {row['sample']:20s} ({row['aging_condition']:12s}, t={row['aging_time_day']:3.0f}d) -> {test_preds[i]:.4f}")

    print(f"\nv32: R2={r2:.4f}, MAE={opt_mae:.4f}")
    ...
    if v14_path.exists():
        ...
        for w14, label in [(0.3, "0.3v14_0.7v32"), (0.5, "0.5v14_0.5v32"), (0.7, "0.7v14_0.3v32")]:
            ens = w14 * v14 + (1 - w14) * test_preds
            pd.DataFrame({TARGET: ens}).to_csv(download_dir / f"predict_out_v14_v32_{label}.csv", index=False)
        print("\n[v14+v32 ensemble] 已生成")
